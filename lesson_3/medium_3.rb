...
def fun_with_ids
  a_outer = 42 # id
  b_outer = "forty two" # id
  c_outer = [42] # id
  d_outer = c_outer[0] # === 42 so has same id as a_outer

  a_outer_id = a_outer.object_id
  b_outer_id = b_outer.object_id
  c_outer_id = c_outer.object_id
  d_outer_id = d_outer.object_id # same id as a_outer
  ...
  1.times do
    a_outer_inner_id = a_outer.object_id # same id, no change
    ...
    a_outer = 22 # points to different object so id changes
    ...
    a_inner = a_outer # points to same object so same id
    b_inner = b_outer
    c_inner = c_outer
    d_inner = c_inner[0]

    a_inner_id = a_inner.object_id # points to same object so same id
...
# ids change because they were reassigned inside the block
  ...
  # cannot be called outside the block as they were initialized inside
end
...
# because the variables were reassigned inside the method, the orignial variables were not changed.

#3) my_string was not mutated, my_array was
#4) a_string_param.gsub!('pumpkins', 'rutabaga') changes the original object
# puts "My array looks like this now: #{my_array}" created a new object
#5)
